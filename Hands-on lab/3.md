...
3. On the **Select source** tab, enter the following:
   ...
   - **Authentication type (2)**: Select **SQL Server Authentication**.
   - **Username (3)**: Enter **PUWebSite**
   - **Password (4)**: Enter **<inject key="SQLVM Password" enableCopy="True" />**
...
3. Paste the value into a text editor, such as Notepad.exe, to replace the Password placeholder. Replace the `{your_password}` section with **<inject key="SQLVM Password" enableCopy="True" />**. Copy the full connection string with the replaced password for later use.

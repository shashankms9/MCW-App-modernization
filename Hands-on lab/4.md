...
1. Create a Personal Access Token as described below.
     ...
     1. Now, Under Personal access tokens click on **Tokens(classic) (1)** and, select **Generate new token (2)** button on the right and then click on  **Generate             new  token(classic) (3)** Provide the GitHub password if prompted.
...
11. Open Windows Explorer. Copy the solution template yml file `C:\MCW\MCW-App-modernization-stage\Hands-on lab\lab-files\workflow\stagingdeploy.yml` to `C:\MCW\MCW-App-modernization-stage\Hands-on lab\lab-files\src\.github\workflows\stagingdeploy.yml`.  You are going to replace the default GitHub workflow yml content.

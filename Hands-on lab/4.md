...
13. Select your subscription **(1)** and the `partsunlimited` Azure Migrate project **(2)**. Select **Upload (3)** to start the upload to Azure.
...
9. On the Migration Wizard **Select source** Blade, enter the following:
   ...
   - Select **Next: Select databases for migartion >>**.
...
13. On the Migration Wizard **Configure migration settings** Blade, expand the **PartsUnlimited** database, verify all the tables are selected and select **Next: Database migration Summary >>**.
    ...
    > **Note**: If you see that table data cannot be migrated, the source table is empty. it is completely fine please select the tables which are not greyed out or table has data in it.
...
1. In the [Azure portal](https://portal.azure.com), navigate to your `parts` SQL Database resource by selecting **Resource groups** from the Azure services list, selecting the **hands-on-lab-<inject key="DeploymentID" enableCopy="false"/>** resource group, and selecting the `parts` SQL Database from the list of resources.
...
6. On the **connection string** panel, enter the following:
   ...
   - **Type (4)**: Select **SQLAzure**
   - Select **Apply and Confirm (5)**.
...
7. Switch to the **Overview (1)** Blade, and select **Default domain (2)** to navigate to the Parts Unlimited website hosted in our Azure App Service using Azure SQL Database.
